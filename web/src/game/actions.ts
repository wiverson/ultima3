...
import { type GameIO, Key, Sound, getKey, getChar, inputNumber, deathSound } from './io.ts';
...
export async function handEquipment(world: World, io: GameIO): Promise<void> {
  ...
  switch (item) {
    ...
    case 'A': {
      ...
      if (what < 'B' || what > last) return error(Msg.None);
